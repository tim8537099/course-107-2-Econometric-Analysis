...
接續上題，課堂上的treatment dummy即為這裡的**可修個體**dummy，請寫下對應課堂效應結構$Y_i=Y_{0i}+(Y_{1i}-Y_{0i})T_i$之對應寫法（以這裡的文字符號表示）。
 ...
 $個經PRi=個經PR{0i}+(個經PR{1i}-個經PR{0i})T_i$
...
library(lmtest)
hw1Data1<- hw1Data %>% 
  rename(microPR =個經PR , microOK = 可修個體, ecoPR = 經原PR , macroPR = 總經PR , macroOK = 可修總體)
model1<- lm(microPR~microOK,hw1Data1)
...
```{r ans16}
lm(microPR~microOK+ecoPR,hw1Data1) ->model2
...
www<- hw1Data1 %>%
  group_by(microOK) %>%
  summarise(ecoPRmean=mean(ecoPR,na.rm=T),microPRmean=mean(microPR,na.rm=T))
```
...
## 1.8 偏誤方向
請填入以下空格完成完整偏誤論述：  
有控制**經原PR**時，擋修會使得**個經PR** (1)__下降__(上升/下降) (2)__1_個_百分點，其值比未控制**經原PR**時還 (3)_低___(高/低)；這表示忽略**經原PR**會對效應係數估計產生 (4)___正向偏誤_(正向偏誤(upward bias)/負向偏誤(downward bias))。之所以會有上述的偏誤方向來自於兩個因素：

  1. **經原PR**與**個經PR**有(5)正向____(正向/反向)關連
  2. **可修個體**=1的族群，其**經原PR**平均較(6)__高__(高/低)

故若不控制**經原PR**，因**可修個體**=1與=0兩群人也會分別帶有 **經原PR**  (7)___高與低_(高與低/低與高)特質，所以**可修個體**=1的人其**個經PR**高，有部份是因其**經原PR**也較(8)__高__(高/低)所至。
...
$$so\frac{1}{[(x_i-\bar{x})^2]^2}\sum_{i=1}^NE[(x_i-\bar{x})^2\epsilon_i^2|x]=\frac{\sigma^2}{\sum_{i=1}^N(x_i-\bar{x)^2}}=\frac{\sigma^2(n_0+n_1)}{n_0n_1}$$
...
$$\frac{1}{[(x_i-\bar{x})^2]^2}\sum_{i=1}^NE[(x_i-\bar{x})^2\epsilon_i^2|x]=\frac{\sigma^2}{\sum_{i=1}^N(x_i-\bar{x)^2}}=\frac{\sigma^2(n_0+n_1)}{n_0n_1}$$
